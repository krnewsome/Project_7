...
//Tweets
class Tweets {
  constructor(tweetContent, retweetNum, likesNum, dateOfTweet, profileImage, scrName, retweetCount, timeCreated, userIdString, userBackgroundBanner){
  ...
  T.get('friends/list', { count }, function (err, data, response) {
    ...
    T.get('statuses/user_timeline', { count }, function (err, data, response) {
      for (let i = 0; i < count; i++) {
        ...
        let tweet = new Tweets(data[i].text, data[i].retweet_count, data[i].favorite_count, data[i].created_at, data[i].user.profile_image_url, data[i].user.screen_name, data[i].user.retweet_count, date, data[i].user.id_str, data[i].user.profile_banner_url)
